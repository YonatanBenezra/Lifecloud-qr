                    ...
                    {profiledata && profiledata.addFriends.length > 0 ? profiledata.addFriends.map((friend, i) => {
                        return (
                            <div className="friend-request" key={friend.user && friend.user[0]._id}>
                                ...
                                <div>
                                    <span style={{ cursor: 'pointer' }} onClick={() => handleAddAcceptFrined(friend)}>הוסף חבר</span>
                                    |
                                    <span style={{ cursor: 'pointer' }} onClick={() => handleAddFriend(friend.user[0]._id)}>הסר חבר</span>
                                </div>
                            </div>
                        )
                    }) : <div style={{ textAlign: 'center' }}>אין עדיין חברים</div>}
                    <h1>חברים</h1>
                        ...
                        return (
                            <div className="friend-request" key={user._id}>
                                ...
                                <div>
                                    ...
                                    {e[i] ? <span style={{ cursor: 'pointer' }}>Friend reqest sent</span> : <span onClick={() => handleAddFriend(user._id)} style={{ cursor: 'pointer' }}>הוסף חבר</span>}



                                    |
                                    {eAdmin[i] ? <span style={{ cursor: 'pointer' }}>Admin</span> : <span onClick={() => handleAddAdmins(user._id)} style={{ cursor: 'pointer' }}>+ הוסף כאדמין</span>}
                    ...
                    {profiledata && profiledata.addAdmins.length > 0 ? profiledata.addAdmins.map((admin, i) => {
                        return (
                            <div className="friend-request" key={admin.user && admin.user[0]._id}>
                                ...
                                <div>
                                    <span onClick={() => handleAddAdmins(admin.user[0]._id)} style={{ cursor: 'pointer' }}>הסר אדמין</span>
                                </div>
                            </div>
                        )
